...
export default function CreateRoomPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col gap-4 p-4">
      <h1 className="text-2xl font-bold text-primary">{uxCopy.room.createTitle}</h1>
      <p className="text-sm text-gray-600">{uxCopy.room.createSubtitle}</p>
      <Card>
        <form action={formAction} className="space-y-3">
          ...
          <Button type="submit" disabled={isPending}>
            {isPending ? uxCopy.room.createPending : uxCopy.room.createCta}
          </Button>
        </form>
      </Card>
